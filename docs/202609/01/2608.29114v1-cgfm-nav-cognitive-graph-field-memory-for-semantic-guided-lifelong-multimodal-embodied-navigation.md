...
tags: ["query:robotics"]
score: 8.0
evidence: 具身导航中的多模态场景图记忆与长时推理
tldr: "视觉语言导航任务要求智能体在持续探索未知区域的同时，基于累积观测进行推理，但现有环境表示难以同时支持显式语义记忆与连续探索引导。为此，我们提出认知图-场记忆（CGFM），将对象、空间关系与视觉观测组织为多模态场景图，支持目标检索与长时序推理；当目标匹配不可靠时，将图证据投影为语义前沿场，引导向语义有潜力的区域探索。基于CGFM构建的CGFM-Nav框架结合子图选择、VLM推理与验证反馈，形成闭环决策。在GOAT-Bench上，相同Qwen3-VL-8B骨干下成功率从53.2%提升至63.0%，SPL从30.0%提升至39.6%，验证了显式语义记忆与语义引导探索结合的有效性。"
source: arxiv
...
motivation: 现有VLN环境表示难以兼顾显式语义记忆与连续探索引导，导致长时序推理和目标检索能力受限。
method: 提出CGFM多模态场景图存储语义关系，并投影为语义前沿场；构建CGFM-Nav框架，集成子图选择、VLM推理与验证反馈闭环。
result: "在GOAT-Bench上，CGFM-Nav将成功率从53.2%提升至63.0%，SPL从30.0%提升至39.6%。"
conclusion: 显式语义记忆与语义引导探索的耦合能有效提升终身多模态导航的性能与泛化能力。
---

## 摘要
视觉与语言导航（VLN）要求智能体在持续探索未知区域的同时，对累积的观测进行推理。然而，现有的环境表示往往难以同时支持显式语义记忆和连续探索引导。为应对这一挑战，我们提出了认知图-场记忆（CGFM），一种将显式关系记忆与连续空间直觉相结合的持久多模态场景表示。CGFM将物体、空间关系和视觉观测组织为多模态场景图，支持跨导航任务的目标检索和长时程推理。当无法识别到可靠的目标匹配时，基于图的证据被投影到目标条件语义前沿场中，以引导探索朝向语义上有希望的前沿和区域。基于CGFM，我们引入了CGFM-Nav，一种面向终身多模态导航的基座模型框架，将任务相关子图选择、VLM推理和验证反馈整合到闭环决策循环中。在GOAT-Bench上的初步实验表明，在相同的Qwen3-VL-8B骨干网络下，CGFM-Nav将整体成功率从53.2%提升至63.0%，SPL从30.0%提升至39.6%，展示了显式语义记忆与语义引导探索相结合的有效性。
