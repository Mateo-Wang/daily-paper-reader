...
- **2026-08-20** 🧭 优化 AI 前沿与精选研究主题更新机制：候选每天无模型收集，周三与周日集中评审；周日用同一次 DeepSeek 调用完成第二篇前沿筛选和近两周主题提炼，并通过候选评审缓存、输入指纹与原子写入降低 Token 消耗、避免重复生成。
- **2026-07-30** ⭐ 上线仓库级收藏并升级论文详情页：收藏结果跨浏览器共享，支持标签、评分、日期筛选、批量取消与 Markdown / CSV 导出；同步重构论文详情页及图表弹窗视觉，原有内容和功能保持不变。
...
论文卡片和详情页均提供“收藏”按钮。收藏结果写入当前仓库的 `data/favorites.json`，因此同一仓库在 Chrome、Edge 或其他设备中打开时会读取同一份收藏清单，不依赖浏览器本地存储。

- 先解锁项目密钥，确保配置的 GitHub PAT 对当前仓库具有 Contents 写入权限。
- 收藏后的论文显示红色五角星，可从左侧“已收藏”入口集中查看。
- 收藏列表支持按标签、评分和日期筛选，可批量取消收藏，并导出 Markdown 或 CSV。
- arXiv 同一论文的不同版本会归一为同一个论文 ID，避免重复写入。
- 每次修改采用 GitHub Contents API 的 SHA 校验与冲突重试，防止并发操作覆盖已有收藏。
